...
def hash_sha3(value: int) -> int:
    byte_data = value.to_bytes(ceil(value.bit_length() / 8), byteorder=byteorder) # Преобразуем число в массив байтов
...
def cast_vote(voter_name: str, choice: str, server: VotingServer, result_output: tk.Text):
    ...
    vote_packet = (random_value << 257) | vote_value
    ...
    s_prime = pow_module(h_prime, server.C, server.N)  # Подписанный
    s = s_prime * modular_inverse(r, server.N) % server.N  # "заслепление"
